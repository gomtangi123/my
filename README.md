...
**모든 카드에 사진이 깔립니다.** `auto`(기본)면 카드 종류가 방식을 정합니다.

| 카드 | 사진 | 왜 |
|---|---|---|
| 표지 | **꽉 채우기** + 아래쪽 그라데이션 장막 | 위쪽 사진을 살려야 합니다 |
| 본문·숫자·마무리 | **꽉 채우기** + 장막 | |
| 표·그래프 | **그라데이션으로 펴기** | 아래 설명 |

`--photos full` / `band` / `wash` / `off` 로 전부 한 가지로 밀 수도 있고,
카드마다 `@사진없음` 으로 끌 수도 있습니다.

#### 표·그래프는 사진을 그라데이션으로

표 뒤에 사진을 그대로 깔면 가는 구분선과 흐린 머리글이 묻힙니다. 그렇다고
사진을 빼면 그 장만 앞뒤와 따로 놉니다.

그래서 **같은 사진을 아주 잘게 줄였다가 다시 키웁니다.** 형태는 사라지고
색과 밝기 배치만 남습니다 — 덱의 흐름은 이어지고 표는 읽힙니다.

#### 잉크도 사진에 맞춰 밉니다

장막 농도는 진한 글자색 기준으로 잡습니다. 그런데 표·그래프는 **흐린 잉크와
회색 막대**를 함께 쓰고, 그것들은 중간 밝기라 같은 배경에서 훨씬 불리합니다.
진한 글씨는 읽히는데 머리글만 사진에 묻히는 일이 실제로 생겼습니다.

장막을 더 진하게 해서 맞추면 사진이 통째로 사라지므로, 장막은 그대로 두고
**잉크를 글자색 쪽으로 밉니다.** 글씨는 4.5:1, 막대·구분선 같은 도형은
3:1 을 기준으로 합니다.
...
| `--photos` | `auto`(기본) · `full` · `band` · `wash` · `off` |
| `--backdrop` | 사진을 못 찾았을 때 쓸 배경. `mesh`(기본) · `grid` · `rays` · `dots` · `off` |
